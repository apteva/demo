...
export function IntegrationCard({ spec, appSlug, serverId, contextProps, pulse }: Props) {
 ...
 {error && (
 <div className="absolute top-1 right-1 text-[11px] text-error bg-error/10 px-1.5 py-0.5 rounded" title={error}>
 ⚠ {loading ? "retrying" :"fetch failed"}
 </div>
 )}
 {loading && !data && (
 <div className="absolute top-1 right-1 text-[11px] text-text-dim">loading…</div>
 )}
 </div>
 );
}
