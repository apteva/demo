...
export function ActivityStrip({ instanceId, onToolCall }: Props) {
 ...
 if (!instanceId) {
 return <div className="text-text-dim text-xs italic">No agent bound — set ?instance=N or pick one.</div>;
 }

 if (items.length === 0) {
 return <div className="text-text-dim text-xs italic">Waiting for agent activity…</div>;
 }
 ...
 <span className="text-text-dim tabular-nums w-12 shrink-0">{it.time}</span>
 <span className="text-text truncate">{it.text}</span>
 </li>
 ))}
 </ul>
 );
